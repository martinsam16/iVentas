...
//                    break;
    ...
    public DefaultTableModel listarPro(char tipo) {
        ...
        try {
            ...
            switch(tipo){
                case '0':
                    sql = "SELECT PRODUCTO.CODPRO, PRODUCTO.NOMPRO, MARCA.NOMMAR, MODELO.NOMMOD, PRODUCTO.SERPRO, PRODUCTO.PREPRO, PRODUCTO.URLIMGPRO, PRODUCTO.ATRIBPRO, PERSONA.NOMPER, PRODUCTO.FECGARPRO,CATEGORIA.NOMCAT, SUM(STOCK.INPSTK)-SUM(STOCK.OUTSTK) AS STOCK FROM MODELO INNER JOIN PRODUCTO ON PRODUCTO.MODELO_CODMOD_MODPRO = MODELO.CODMOD INNER JOIN MARCA ON MODELO.MARCA_CODMAR_MARMOD = MARCA.CODMAR INNER JOIN PERSONA ON PRODUCTO.PERSONA_CODPER_PROVPRO = PERSONA.CODPER INNER JOIN CATEGORIA ON PRODUCTO.CATEGORIA_CODCAT_CATPRO = CATEGORIA.CODCAT LEFT JOIN STOCK ON STOCK.PRODUCTO_CODPRO_PROSTK = PRODUCTO.CODPRO GROUP BY PRODUCTO.CODPRO";

            clms = "CÓDIGO,NOMBRE,MARCA,MODELO,SERIE,PRECIO,URLIMG,ATRIB,PROVEEDOR,GAR,CATEGORIA,STOCK";
                    break;
                
                case '1':
                    sql = "SELECT STOCK.CODSTK, STOCK.FECINP, STOCK.FECOUT, PRODUCTO.NOMPRO, STOCK.INPSTK-STOCK.OUTSTK AS CANTIDAD FROM STOCK INNER JOIN PRODUCTO ON STOCK.PRODUCTO_CODPRO_PROSTK = PRODUCTO.CODPRO GROUP BY STOCK.CODSTK";
                    clms= "CODIGO,FECHA INPUT,FECHA OUTPUT,PRODUCTO,CANTIDAD";
                    break;
            }
